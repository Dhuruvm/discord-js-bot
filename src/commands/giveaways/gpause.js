...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    if (giveaway.pauseOptions?.isPaused) {
      return message.safeReply("<:error:1424072711671382076> This giveaway is already paused!");
    }

    try {
      await giveaway.pause();
      return message.safeReply("<:success:1424072640829722745> Giveaway paused successfully!");
    } catch (error) {
  ...
  async interactionRun(interaction) {
    ...
    if (giveaway.pauseOptions?.isPaused) {
      return interaction.followUp("<:error:1424072711671382076> This giveaway is already paused!");
    }

    try {
      await giveaway.pause();
      return interaction.followUp("<:success:1424072640829722745> Giveaway paused successfully!");
    } catch (error) {
