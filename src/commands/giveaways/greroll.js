...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    if (!giveaway.ended) {
      return message.safeReply("<:error:1424072711671382076> This giveaway hasn't ended yet! Use `gend` to end it first.");
    }

    try {
      await giveaway.reroll();
      return message.safeReply("<:success:1424072640829722745> Giveaway rerolled successfully!");
    } catch (error) {
  ...
  async interactionRun(interaction) {
    ...
    if (!giveaway.ended) {
      return interaction.followUp("<:error:1424072711671382076> This giveaway hasn't ended yet! Use `/gend` to end it first.");
    }

    try {
      await giveaway.reroll();
      return interaction.followUp("<:success:1424072640829722745> Giveaway rerolled successfully!");
    } catch (error) {
